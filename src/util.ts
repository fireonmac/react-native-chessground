import type { Color, Key, Piece } from './types';
...
export const allPosAndKey: readonly { key: Key; pos: Pos }[] = allKeys.map(
  (key, i) => ({
    key,
    pos: allPos[i]!,
  })
...
export const opposite = (c: Color): Color =>
  c === 'white' ? 'black' : 'white';
...
export const samePos = (p1: Pos, p2: Pos): boolean =>
  p1[0] === p2[0] && p1[1] === p2[1];
...
export const knightDir: DirectionalCheck = (x1, y1, x2, y2) =>
  diff(x1, x2) * diff(y1, y2) === 2;

export const rookDir: DirectionalCheck = (x1, y1, x2, y2) =>
  (x1 === x2) !== (y1 === y2);
